...
func TestGetListOptions(t *testing.T) {
	req, err := http.NewRequest("GET", "https://test.url/api", nil)
	assert.Empty(t, err)

	req.URL.RawQuery = "_raw_query=true&fieldSelector=type!=kubernetes.io/service-account-token"
	options, err := getListOption(req)
	expected := &metav1.ListOptions{
		FieldSelector: "type!=kubernetes.io/service-account-token",
	}
	assert.Empty(t, err)
	assert.Equal(t, options, expected, "unexpected list options")
}
